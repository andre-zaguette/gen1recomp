...
GameVersion.VERSIONS = {
  ...
  crystal = {
    ...
    romSize = 2 * 1024 * 1024,
    -- Its own skeleton extractor (src/import/RomExtractorCrystal.lua), not
    -- Gold's full src/import/RomExtractorGen2.lua -- the two Gen 2 carts
    -- disagree on layout below the constants layer just like Gold disagrees
    -- with Gen 1, so each ROM gets its own extractor module.
    extractor = "crystal",
    -- Generation-aware systems (mod targeting, expanded party storage, link
    -- fingerprinting -- see GameVersion.generation below) need this to treat
    -- Crystal the same way as Gold rather than falling back to gen 1.
    generation = 2,
  },
...
GameVersion.ORDER = { "red", "blue", "yellow", "gold", "crystal" }
